...
RUN         apk add --update -t build-deps curl go git libc-dev gcc libgcc ansible util-linux sqlite bash curl mysql-client jq openssh openssl wget && \
            ...
            rm -rf /var/cache/apk/* && \
            rm -rf ${GOPATH}
RUN 	    cd ~/ && wget https://github.com/rancher/rancher-compose/releases/download/v0.8.5-rc2/rancher-compose-linux-amd64-v0.8.5-rc2.tar.gz && tar xzvf rancher-compose-linux-amd64-v0.8.5-rc2.tar.gz && \
